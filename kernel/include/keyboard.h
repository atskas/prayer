...
extern volatile char keyboard_buffer[KB_BUFFER_SIZE]; // The global keyboard buffer.
extern volatile uint32_t keyboard_len;

char get_char_from_buffer(); // Get a character from the global keyboard buffer.
void read_line(char* buf, int maxLen); // Read a line from the global keyboard buffer.
